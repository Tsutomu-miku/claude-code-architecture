...
const capGridStyle: React.CSSProperties = {
  display: 'grid',
...
const highlightsHeadingStyle: React.CSSProperties = {
  fontSize: '1.35rem',
  fontWeight: 700,
  letterSpacing: '-0.02em',
...
const highlightsSubStyle: React.CSSProperties = {
  fontSize: '0.9rem',
  color: 'var(--text-dim)',
  textAlign: 'center' as const,
  marginBottom: '36px',
  ...
  return (
    <section id="hero" className="section hero">
      ...
      <motion.div
        ...
        transition={{ duration: 0.6 }}
      >
        <div style={taglineStyle}>Project Overview</div>
        <div style={overviewBoxStyle}>
          ...
          <p style={overviewTextStyle}>
            Claude Code 是由 Anthropic 开发的 <strong style={{ color: 'var(--accent-red)' }}>AI 编程助手命令行工具</strong>。
          ...
          <p style={overviewTextStyle}>
            从技术实现来看，Claude Code 使用 <strong style={{ color: 'var(--text)' }}>TypeScript</strong> 编写，
            运行在 <strong style={{ color: 'var(--text)' }}>Bun</strong> 运行时之上，
            终端 UI 基于 <strong style={{ color: 'var(--text)' }}>React + Ink</strong> 渲染，
            CLI 参数解析使用 <strong style={{ color: 'var(--text)' }}>Commander.js</strong>。
          ...
          <div style={{ ...overviewHeadingStyle, marginTop: '28px' }}>
            <span style={{ fontSize: '1.4rem' }}>🎯</span>
            核心能力一览
          </div>
          <p style={{ ...overviewTextStyle, marginBottom: '12px' }}>
            Claude Code 内置了 40 多个工具和 50 多个斜杠命令，覆盖了从代码编写到项目管理的完整开发流程：
          </p>
          <div style={capGridStyle}>
            {capabilities.map((cap, i) => (
              <div key={i} style={capItemStyle}>
                <div>
                  <div style={capLabelStyle}>▸ {cap.label}</div>
                  <div style={capDetailStyle}>{cap.detail}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <motion.div
        ...
        transition={{ duration: 0.6 }}
      >
        <div style={taglineStyle}>Core Highlights</div>
        <h3 style={highlightsHeadingStyle}>核心架构亮点</h3>
        <p style={highlightsSubStyle}>
        ...
        >
          {highlights.map((h, i) => (
            <HighlightCard key={i} icon={h.icon} title={h.title} desc={h.desc} index={i} />
          ))}
        </motion.div>
      </motion.div>
    </section>
...
function HighlightCard({
  ...
  const borderColor = hovered ? 'var(--accent-purple)' : 'var(--border)'
  const transform = hovered ? 'translateY(-4px)' : 'translateY(0)'
  const shadow = hovered ? '0 8px 32px rgba(0,0,0,0.25)' : 'none'
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
        ...
        <div>
          ...
          <div style={highlightTitleStyle}>{title}</div>
        </div>
      </div>
      <p style={highlightDescStyle}>{desc}</p>
    </motion.div>
  )
}
